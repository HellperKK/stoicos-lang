...
const prelude = () => {
    ...
    // Imports
    vars.setVar("import", FunToken.native(toks => {
    ...
    // Constructors
    vars.setVar("fun", FunToken.native(toks => {
    ...
    // IO
    vars.setVar("println", FunToken.native(toks => {
        stdOut.content += toks[0].request("string") + "\n"
        return VarManager.unit
    }), true)
    vars.setVar("print", FunToken.native(toks => {
        stdOut.content += toks[0].request("string")
        return VarManager.unit
    }), true)
    ...
    // Defining
    vars.setVar("def", FunToken.native(toks => {
        const name = toks[0].request("symbol")
        const value = toks[1]
        vars.setVar(name, value, false)
        return value
    }), true)
    vars.setVar("deffun", FunToken.native(toks => {
        const name = toks[0].request("symbol")
