...
const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    {"name": "phrase", "symbols": [(lexer.has("openPar") ? {type: "openPar"} : openPar), "_", "vallist", "_", (lexer.has("closePar") ? {type: "closePar"} : closePar), "_"], "postprocess": function(arr) { const list = arr[2].filter(x => x); return new CallToken(list[0], list.slice(1)) }},
