...
const grammar: Grammar = {
	...
	ParserRules: [
		...
		{ name: "main", symbols: ["main$ebnf$1"], postprocess: id },
		{
			...
			postprocess: (arr) => {
				const list = arr[2].filter((x) => x);
				return new CallToken(list.shift(), list);
			},
		},
		{
			name: "phrase",
		...
		},
		{
			...
			postprocess: (arr) => arr[0].concat([arr[2]]),
		},
		{ name: "vallist", symbols: ["value"], postprocess: (arr) => [arr[0]] },
		{
			name: "value",
		...
		{
			name: "var",
			symbols: ["name", lexer.has("dot") ? { type: "dot" } : dot, "attrlist"],
			postprocess: (arr) => {
				return new AttrToken(
					arr[0].value,
					arr[2].map((attr) => attr.value),
				);
			},
		},
		{
		...
		{ name: "attrlist", symbols: ["name"], postprocess: (arr) => [arr[0]] },
		{
			name: "curried",
			symbols: [lexer.has("hash") ? { type: "hash" } : hash, "var"],
			postprocess: (arr) => new PartialFunToken(arr[1]),
		},
		{
			name: "macro",
			symbols: [lexer.has("dollar") ? { type: "dollar" } : dollar, "value"],
			postprocess: (arr) => ({ type: "macro", value: arr[1] }),
		},
		...
		{ name: "block$ebnf$1", symbols: [], postprocess: () => null },
		{
			...
			postprocess: (arr) => new BlockToken((arr[2] ?? []).filter((x) => x)),
		},
		{
			name: "block",
		...
		{ name: "array$ebnf$1", symbols: [], postprocess: () => null },
		{
			...
			postprocess: (arr) =>
				new ArrayModelToken((arr[2] ?? []).filter((x) => x)),
		},
		{
			name: "name",
